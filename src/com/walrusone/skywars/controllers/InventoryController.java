            ...
            if (playerInventories.exists()) {
                ...
                if (storage.isSet("inventories")) {
                    for (String uuid : storage.getConfigurationSection("inventories.").getKeys(false)) {
                        ItemStack[] content = ((List<ItemStack>) storage.get("inventories." + uuid + ".contents")).toArray(new ItemStack[((List<ItemStack>) storage.get("inventories." + uuid + ".contents")).size()]);
                        ItemStack[] armor = ((List<ItemStack>) storage.get("inventories." + uuid + ".armor")).toArray(new ItemStack[((List<ItemStack>) storage.get("inventories." + uuid + ".armor")).size()]);
