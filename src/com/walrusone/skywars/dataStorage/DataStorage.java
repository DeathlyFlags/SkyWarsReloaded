        ...
        if (!sqlEnabled) {
            ...
            try {
                String queryBuilder = "UPDATE `swreloaded_player` SET " +
                        ...
                        "`games_won` = ?, `kills` = ?, " +
                        "`deaths` = ?, `killdeath` = ?, `blocksplaced` = ?, `last_seen` = NOW(), `balance` = ?, `glasscolor` = ?, `effect` = ?, `traileffect` = ? " +
                        "WHERE `uuid` = ?;";
        ...
        if (sqlEnabled) {
            Bukkit.getScheduler().runTaskAsynchronously(SkyWarsReloaded.get(), () -> {
                ...
                if (database.doesPlayerExist(player.getP().getUniqueId().toString())) {
                    ...
                    try {
                        String queryBuilder = "SELECT `score`, `games_played`, `games_won`, `kills`, `deaths`, `blocksplaced`, `balance`, `glasscolor`, `effect`, `traileffect` " +
                                "FROM `swreloaded_player` " +
                                "WHERE `uuid` = ? " +
                                "LIMIT 1;";
                ...
                try {
                    String queryBuilder = "SELECT `permissions` " +
                            "FROM `swreloaded_permissions` " +
                            "WHERE `uuid` = ?;";
