...
public class Game {
    ...
    private final List<GamePlayer> gPlayers = new ArrayList<>();
    private final List<GamePlayer> spectators = new ArrayList<>();
    private final Map<GamePlayer, Integer> kills = new HashMap<>();
    private final int gameNumber;
    private final int min;
    private final int max;
    private final Map<Integer, GamePlayer> availableSpawns = new HashMap<>();
    private final Map<String, Integer> scoreboardData = new HashMap<>();
    ...
    public Game(int gameNumber, String map) {
        ...
        if (SkyWarsReloaded.getCfg().getSpawn() == null) {
            SkyWarsReloaded
                    .get()
                    .getLogger()
                    .info("YOU MUST SET SPAWN IN THE LOBBY WORLD WITH /SWR SETSPAWN BEFORE STARTING A GAME");
            endGame();
        }
        gameState = GameState.PREGAME;
        prepareGameMap();
        prepareScoreBoard();
        @SuppressWarnings("UnusedAssignment") BukkitTask checkMinPlayerTask = new CheckForMinPlayers(this).runTaskTimer(SkyWarsReloaded.get(),
                20, 20);
    }
    ...
    private void prepareGameMap() {
        ...
        mapWorld = SkyWarsReloaded.get().getServer()
                .getWorld(mapName + "_" + this.gameNumber);
        if (gameLoad) {
            ...
            minPlayers = numberOfSpawns
                    * SkyWarsReloaded.getCfg().getMinPercentPlayers() / 100;
            if (minPlayers < 2) {
                minPlayers = 2;
            }
    ...
    public void addPlayer(GamePlayer gPlayer) {
        ...
        if (SkyWarsReloaded.getCfg().resetTimerEnabled()) {
            if (SkyWarsReloaded.getCfg().getResetTimerThreshold() >= (gPlayers
                    .size() / numberOfSpawns)) {
                count = 0;
            }
        }
    ...
    private void preparePlayerForLobby(final GamePlayer gPlayer) {
        ...
        location = new Location(mapWorld,
                gameMap.getSpawns().get(spawn).getX() + 0.5, gameMap
                .getSpawns().get(spawn).getY() + 1, gameMap.getSpawns()
                .get(spawn).getZ() + 0.5);
        ...
        SkyWarsReloaded.get().getServer().getScheduler()
                .scheduleSyncDelayedTask(SkyWarsReloaded.get(), () -> {
                    if (gPlayer.getP() != null) {
                        ...
                        SkyWarsReloaded.getNMS().sendTitle(
                                gPlayer.getP(),
                                new Messaging.MessageFormatter()
                                        .setVariable("mapname", mapName)
                                        .formatNoColor(
                                                "titles.joinGameTitle"),
                                SkyWarsReloaded.getMessaging().getMessage(
                                        "titles.joinGameSubtitle"));
                        for (PotionEffect effect : gPlayer.getP()
                                .getActivePotionEffects()) {
                            gPlayer.getP().removePotionEffect(
                                    effect.getType());
                        }
                        gPlayer.getP().setGameMode(GameMode.ADVENTURE);
                        scoreboardData.put(gPlayer.getP().getName(), 1);
                        gPlayer.getP().setScoreboard(
    ...
    private void killMobs() {
        mapWorld.getEntities().stream().filter(entity -> entity instanceof Monster).forEach(Entity::remove);
    }
    ...
    private void fillChests() {
        ...
        String vote = SkyWarsReloaded.getCfg().getDefaultChest();
        ...
        if (votesForBasic > votesForNormal && votesForBasic > votesForOP) {
            vote = "basic";
        } else if (votesForNormal > votesForBasic
                && votesForNormal > votesForOP) {
            vote = "normal";
        } else if (votesForOP > votesForBasic && votesForOP > votesForNormal) {
            vote = "op";
        }

        for (EmptyChest eChest : gameMap.getChests()) {
            Location loc = new Location(mapWorld, eChest.getX(), eChest.getY(),
                    eChest.getZ());
    ...
    public void prepareForStart() {
        if (gameState == GameState.PREGAME) {
            if (gPlayers.size() >= minPlayers && gPlayers.size() > 1) {
                if (count == 0) {
                    sendGameMessage(new Messaging.MessageFormatter()
                        ...
                        && count < SkyWarsReloaded.getCfg().preGameTimer()) {
                ...
                updateExpBar(SkyWarsReloaded.getCfg().preGameTimer() - count);
                count++;
            } else
                count = 0;
        ...
        if (gameState == GameState.PREGAME || gameState == GameState.PLAYING) {
            ...
            if (sanityChecker == 5) {
                ...
                for (GamePlayer gPlayer : getPlayers()) {
                    if (gPlayer.getP() != null) {
                        if (!gPlayer.getP().getWorld().getName()
        ...
        if (thunderStorm) {
            if (strikeCounter == nextStrike) {
                ...
                if (hitPlayer <= SkyWarsReloaded.getCfg().getStrikeChance()) {
                    ...
                    Player player = gPlayers.get(getRandomNum(size - 1, 0))
                            .getP();
                    ...
                    mapWorld.strikeLightningEffect(new Location(mapWorld, x, y,
                            z));
                }
        ...
        if (gameState == GameState.PLAYING && !forceEnd) {
            ...
            if ((SkyWarsReloaded.getCfg().getMaxGameLength() - gameLength) == 300) {
                ...
                for (GamePlayer gplayer : getPlayers()) {
                    if (winner != gplayer) {
                        gplayer.getP().teleport(
                                new Location(mapWorld, 0, -64, 0),
    ...
    public void startGame() {
        ...
        fillChests();
        removeSpawnHousing();
        getPlayers().stream().filter(gPlayer -> gPlayer.getP() != null).forEach(gPlayer -> {
            gPlayer.getP().setGameMode(GameMode.SURVIVAL);
            gPlayer.getP().getInventory()
                    .remove(SkyWarsReloaded.getCfg().getKitMenuItem());
            gPlayer.getP().getInventory()
                    .remove(SkyWarsReloaded.getCfg().getOptionsItem());
            gPlayer.getP().getInventory()
                    .remove(SkyWarsReloaded.getCfg().getExitGameItem());
            gPlayer.getP().getOpenInventory().close();
            gPlayer.getP().setHealth(20);
            gPlayer.getP().setFoodLevel(20);
            gPlayer.getP().addPotionEffect(
                    new PotionEffect(PotionEffectType.DAMAGE_RESISTANCE,
                            100, 5));
            if (gPlayer.hasKitSelected()) {
                SkyWarsReloaded.getKC().populateInventory(
                        gPlayer.getP().getInventory(),
                        gPlayer.getSelectedKit());
                SkyWarsReloaded.getKC().givePotionEffects(gPlayer,
                        gPlayer.getSelectedKit());
...
//        int puTime = SkyWarsReloaded.getCfg().getSpawnPowerUpsTime();

//        if (puTime > 0)
//            setTask(new PowerUpTask(this).runTaskTimer(SkyWarsReloaded.get(), puTime * 20, puTime * 20));
    }
    ...
    public void endGame() {
        ...
        mapWorld.getPlayers().stream().filter(player -> player != null).forEach(player -> player.teleport(SkyWarsReloaded.getCfg().getSpawn(),
                TeleportCause.PLUGIN));

        mapWorld.getEntities().stream().filter(entity -> entity != null).forEach(Entity::remove);
        if (!SkyWarsReloaded.getCfg().spectatingEnabled() && !shutdown) {
            SkyWarsReloaded
                    ...
                    .getScheduler()
                    .scheduleSyncDelayedTask(SkyWarsReloaded.get(),
                            this::deleteGame, 20 * SkyWarsReloaded.getCfg().getTimeAfterGame());
        } else {
            deleteGame();
        }
    }
    ...
    private void prepareScoreBoard() {
        ...
        ScoreboardManager manager = SkyWarsReloaded.get().getServer()
                .getScoreboardManager();
        scoreboard = manager.getNewScoreboard();
        objective = scoreboard.registerNewObjective("info", "dummy");
        objective.setDisplaySlot(DisplaySlot.SIDEBAR);
        String leaderboard = new Messaging.MessageFormatter().setVariable(
                "mapname", mapName.toUpperCase()).format(
    ...
    private void updateScoreboard() {
        ...
        if (SkyWarsReloaded.getCfg().usePlayerNames()) {
            ...
            Score score = objective.getScore(new Messaging.MessageFormatter()
                    .format("game.scoreboard-players"));
                        ...
                        .scheduleSyncDelayedTask(SkyWarsReloaded.get(),
                                () -> {
                                    SkyWarsReloaded.getInvC()
                                            .restoreInventory(
                                                    gplayer.getP());
                                    BungeeUtil.connectToServer(gplayer
                                            .getP(), SkyWarsReloaded
                                            .getCfg().getLobbyServer());
                                }, 5);
            }
        } else {
            if (gplayer.getP() != null) {
    ...
    private void playerQuit(GamePlayer gPlayer) {
        ...
        if (gameState == GameState.PLAYING) {
                    ...
                    if (killer != gPlayer) {
                        ...
                        if (killer.getP() != null) {
                            if (killer.getP().hasPermission("swr.vip")) {
                                killTotal = SkyWarsReloaded.getCfg()
                                        .getKillValue()
                                        * SkyWarsReloaded.getCfg()
                                        .getVIPMultiplier();
                            } else {
                        ...
                        gPlayer.setScore(gPlayer.getScore()
                                - SkyWarsReloaded.getCfg().getDeathValue());
                        addBalance(killer, killTotal);
                        removeBalance(gPlayer, SkyWarsReloaded.getCfg()
                                .getDeathValue());
                        sendGameMessage(new Messaging.MessageFormatter()
                                .withPrefix()
                                ...
                                .setVariable(
                                        "player_score",
                                        StringUtils
                                                .formatScore(-SkyWarsReloaded
                                                        .getCfg()
                                                        .getDeathValue()))
                                .setVariable("killer", killer.getName())
                                .setVariable("killer_score",
                                        StringUtils.formatScore(killTotal))
                                .format("game.death.quit-while-tagged"));
                    }
                } else {
                    gPlayer.setScore(gPlayer.getScore()
                            - SkyWarsReloaded.getCfg().getLeaveValue());
                    removeBalance(gPlayer, SkyWarsReloaded.getCfg()
                            .getLeaveValue());
                    sendGameMessage(new Messaging.MessageFormatter()
                            .withPrefix()
                            ...
                            .setVariable(
                                    "score",
                                    StringUtils.formatScore(-SkyWarsReloaded
                                            .getCfg().getLeaveValue()))
            ...
            updateScoreboard();
            availableSpawns.keySet().stream().filter(spawn -> availableSpawns.get(spawn) == gPlayer).forEach(spawn -> availableSpawns.put(spawn, null));
            sendGameMessage(new Messaging.MessageFormatter().withPrefix()
                    .setVariable("number", "" + (gPlayers.size()))
                    .setVariable("total", "" + numberOfSpawns)
                    .setVariable("player", gPlayer.getName())
    ...
    private void preparePlayerForExit(GamePlayer gplayer) {
        if (gplayer.getP() != null) {
            ...
            gplayer.setJumpVote(0);
            gplayer.getP().setScoreboard(
                    SkyWarsReloaded.get().getServer().getScoreboardManager()
                            .getNewScoreboard());
    ...
    public Boolean playerExists(GamePlayer name) {
        for (GamePlayer gPlayer : gPlayers) {
            if (name.getUUID().toString()
                    .equalsIgnoreCase(gPlayer.getUUID().toString())) {
                return true;
            }
        }
        return false;
    ...
    public ArrayList<GamePlayer> getPlayers() {
        return new ArrayList<>(gPlayers);
    }

    private ArrayList<GamePlayer> getSpectators() {
        return new ArrayList<>(spectators);
    }

    private Set<GamePlayer> getAllPlayers() {
        Set<GamePlayer> gPlayers = new HashSet<>();
        gPlayers.addAll(getPlayers());
        gPlayers.addAll(getSpectators());
    ...
    private void deleteWorldGuardFolder(String name) {
        File workingDirectory = new File(SkyWarsReloaded.get().getServer()
                .getWorldContainer().getAbsolutePath());
    ...
    public Boolean isFull() {
        return gPlayers.size() >= numberOfSpawns;
    }
    ...
    private void removeSpawnHousing() {
        for (int spawn : gameMap.getSpawns().keySet()) {
            int x = gameMap.getSpawns().get(spawn).getBlockX();
            int y = gameMap.getSpawns().get(spawn).getBlockY();
            int z = gameMap.getSpawns().get(spawn).getBlockZ();
    ...
    public void onPlayerDeath(final GamePlayer target,
                              final DamageCause dCause, final Location loc) {
        ...
        if (SkyWarsReloaded.getCfg().spectatingEnabled())
            target.spectateMode(true, this, target.getP().getLocation(),
                    shutdown);
        ...
        SkyWarsReloaded.get().getServer().getScheduler()
                .scheduleSyncDelayedTask(SkyWarsReloaded.get(), () -> {
                    if (target.getP() != null) {
                        SkyWarsReloaded.getNMS().respawnPlayer(
                                target.getP());
                    }
                    if (!SkyWarsReloaded.getCfg().spectatingEnabled()) {
                    ...
                    if ((System.currentTimeMillis() - target.getTagged()
                            .getTime()) < 10000) {
                        ...
                        if (killer != target) {
                            ...
                            int killTotal = SkyWarsReloaded.getCfg()
                                    .getKillValue();
                            if (killer.getP() != null) {
                                if (killer.getP().hasPermission("swr.vip")) {
                                    killTotal = SkyWarsReloaded.getCfg()
                            ...
                            killer.setScore(killer.getScore() + killTotal);
                            target.setScore(target.getScore()
                                    - SkyWarsReloaded.getCfg()
                                    .getDeathValue());
                            addBalance(killer, killTotal);
                            removeBalance(target, SkyWarsReloaded.getCfg()
                                    .getDeathValue());
                            target.setTagged(target);
                            if (target.getP() != null) {
                                target.getP().sendMessage(
                                        getDeathMessage(dCause, true,
                                                target, killer));
                                target.getP().playSound(
                                        target.getP().getLocation(),
                                        SkyWarsReloaded.getCfg()
                                                .getDeathSound(), 1, 1);
                            }
                            sendGameMessage(getDeathMessage(dCause, true,
                                    target, killer));
                            playSound(SkyWarsReloaded.getCfg()
                                    .getDeathSound());
                        }
                    } else {
                        target.setScore(target.getScore()
                        ...
                        removeBalance(target, SkyWarsReloaded.getCfg()
                                .getDeathValue());
                        sendGameMessage(getDeathMessage(dCause, false,
                                target, target));
        ...
        if (gameState == GameState.PLAYING) {
            if (getPlayers().size() == 1) {
                ...
                if (gPlayer.getP() != null) {
                    if (gPlayer.getP().hasPermission("swr.vip")) {
                        winTotal = SkyWarsReloaded.getCfg().getWinValue()
                                * SkyWarsReloaded.getCfg().getVIPMultiplier();
                    } else {
                        winTotal = SkyWarsReloaded.getCfg().getWinValue();
                    }
                }
                ...
                Bukkit.getServer().getPluginManager()
                        .callEvent(new GameEndEvent(gPlayer.getP(), mapName));
                if (SkyWarsReloaded.getCfg().spectatingEnabled()) {
                    if (gPlayer.getP() != null) {
                        gPlayer.spectateMode(true, this, gPlayer.getP()
                                .getLocation(), shutdown);
                        sendGameMessage(new Messaging.MessageFormatter()
                                .withPrefix()
                                .setVariable(
                                        "time",
                ...
                if (SkyWarsReloaded.getCfg().WinBroadcastDisabled()) {
                    sendGameMessage(new Messaging.MessageFormatter()
                            .setVariable("player", gPlayer.getName())
                            .withPrefix()
                            .setVariable("player_score",
                                    StringUtils.formatScore(winTotal))
                            .setVariable("mapname", mapName).format("game.win"));
                } else {
                    for (GamePlayer gamePlayer : SkyWarsReloaded.getPC()
                            .getAll()) {
    ...
    private void sendGameMessage(String message) {
        getAllPlayers().stream().map(GamePlayer::getP)
                .forEach(p -> p.sendMessage(message));
    }

    public void playSound(Sound sound) {
        getAllPlayers().stream().map(GamePlayer::getP)
                .forEach(p -> p.playSound(p.getLocation(), sound, 1, 1));
    }

    private String getDeathMessage(DamageCause dCause, boolean withHelp,
                                   GamePlayer target, GamePlayer killer) {
        String first;
        int killTotal = SkyWarsReloaded.getCfg().getKillValue();
        if (killer.getP() != null) {
            if (killer.getP().hasPermission("swr.vip")) {
        ...
        if (dCause.equals(DamageCause.BLOCK_EXPLOSION)
                || dCause.equals(DamageCause.ENTITY_EXPLOSION)) {
            first = new Messaging.MessageFormatter()
                    .withPrefix()
                    .setVariable("player", target.getName())
                    .setVariable(
                            "player_score",
                            StringUtils.formatScore(-SkyWarsReloaded.getCfg()
                                    .getDeathValue()))
        ...
        } else if (dCause.equals(DamageCause.FIRE)
                || dCause.equals(DamageCause.FIRE_TICK)) {
            first = new Messaging.MessageFormatter()
                    .withPrefix()
                    .setVariable("player", target.getName())
                    .setVariable(
    ...
    private void launchFireworkDisplay(final World w, final Location loc) {
        Firework fw = w.spawn(
                loc.clone()
                        .add(new Vector(getRandomNum(5, -5), 1, getRandomNum(5,
                                -5))), Firework.class);
        FireworkMeta meta = fw.getFireworkMeta();
        FireworkEffect effect = SkyWarsReloaded.getNMS().getFireworkEffect(
                getRandomColor(), getRandomColor(), getRandomColor(),
                getRandomColor(), getRandomColor(), getRandomType());
    ...
    public void removeSpectator(GamePlayer gPlayer) {
        ...
        if (gPlayer.getP() != null) {
            ...
            if (SkyWarsReloaded.getCfg().bungeeEnabled())
                BungeeUtil.connectToServer(gPlayer.getP(), SkyWarsReloaded
                        .getCfg().getLobbyServer());

            gPlayer.spectateMode(false, this, SkyWarsReloaded.getCfg()
                    .getSpawn(), shutdown);
    ...
    public void sendSpectatorMessage(Player player, String message) {
        GamePlayer sender = SkyWarsReloaded.getPC().getPlayer(
                player.getUniqueId());
        ...
        getSpectators().stream().filter(gPlayer -> gPlayer.getP() != null).forEach(gPlayer -> gPlayer.getP().sendMessage(
                new Messaging.MessageFormatter()
    ...
    private String getTime() {
        ...
        if (votesForDawn >= votesForNoon && votesForDawn >= votesForDusk
                && votesForDawn >= votesForMidnight) {
            return "dawn";
        } else if (votesForNoon >= votesForDusk
                && votesForNoon >= votesForMidnight) {
            return "noon";
    ...
    private void setWeather() {
        ...
        final World world = SkyWarsReloaded.get().getServer()
                .getWorld(mapName + "_" + gameNumber);
        if (weather.equalsIgnoreCase("sunny")) {
            ...
            SkyWarsReloaded.getNMS().updateChunks(chunks);
            world.setStorm(true);
            world.setWeatherDuration(Integer.MAX_VALUE);
        }
    }
    ...
    private String getWeather() {
        ...
        if (votesForSunny >= votesForRain && votesForSunny >= votesForThunder
                && votesForSunny >= votesForSnow) {
            return "sunny";
        } else if (votesForRain >= votesForThunder
                && votesForRain >= votesForSnow) {
            return "rain";
    ...
    private void setJump() {
        ...
        if (jump.equalsIgnoreCase("normal")) {
            ...
            getPlayers().stream().filter(gPlayer -> gPlayer.getP() != null).forEach(gPlayer -> gPlayer.getP().addPotionEffect(
                    new PotionEffect(PotionEffectType.JUMP,
                            Integer.MAX_VALUE, SkyWarsReloaded.getCfg()
                            .getHighJumpValue())));
    ...
    private String getJump() {
        ...
        if (votesForNormal >= votesForHigh && votesForNormal >= votesForSuper
                && votesForNormal >= votesForGod) {
            return "normal";
    ...
    private void removeBalance(GamePlayer p, int x) {
        if (SkyWarsReloaded.getCfg().usingExternalEcomony()) {
            SkyWarsReloaded.econ.withdrawPlayer(SkyWarsReloaded.get()
                    .getServer().getOfflinePlayer(p.getUUID()), x);
        } else {
    ...
    private void addBalance(GamePlayer p, int x) {
        if (SkyWarsReloaded.getCfg().usingExternalEcomony()) {
            SkyWarsReloaded.econ.depositPlayer(SkyWarsReloaded.get()
                    .getServer().getOfflinePlayer(p.getUUID()), x);
        } else {
            p.setBalance(p.getBalance() + x);
        }
    ...
    private int getPlayerSpawn(GamePlayer gPlayer) {
        for (int spawn : availableSpawns.keySet()) {
    ...
    public void setGlass(GamePlayer gPlayer) {
        String color = gPlayer.getGlass();
        if (color == null) {
            color = "normal";
        }
        if (!color.equalsIgnoreCase("normal")) {
            GlassColor colorGlass = SkyWarsReloaded
                    .getGLC().getByColor(color);
            if (colorGlass != null) {
                setGlass(colorGlass.getMaterial(),
                        colorGlass.getData(), gPlayer);
            } else {
    ...
    private void setGlass(@SuppressWarnings("SameParameterValue") Material color, GamePlayer gPlayer) {
        setGlass(color, (byte) 0, gPlayer);
    }
    ...
    public void setGlass(Material color, byte data, GamePlayer gPlayer) {
        if (containsPlayer(gPlayer.getP()) && gameState == GameState.PREGAME) {
            ...
            mapWorld.getBlockAt(x, y, z).setType(color);
            mapWorld.getBlockAt(x, y + 1, z + 1).setType(color);
            mapWorld.getBlockAt(x, y + 1, z - 1).setType(color);
            mapWorld.getBlockAt(x + 1, y + 1, z).setType(color);
            mapWorld.getBlockAt(x - 1, y + 1, z).setType(color);
            mapWorld.getBlockAt(x, y + 2, z + 1).setType(color);
            mapWorld.getBlockAt(x, y + 2, z - 1).setType(color);
            mapWorld.getBlockAt(x + 1, y + 2, z).setType(color);
            mapWorld.getBlockAt(x - 1, y + 2, z).setType(color);
            mapWorld.getBlockAt(x, y + 3, z + 1).setType(color);
            mapWorld.getBlockAt(x, y + 3, z - 1).setType(color);
            mapWorld.getBlockAt(x + 1, y + 3, z).setType(color);
            mapWorld.getBlockAt(x - 1, y + 3, z).setType(color);
            mapWorld.getBlockAt(x, y + 4, z).setType(color);
    ...
    private void updateExpBar(Integer lvl) {
        for (GamePlayer gpl : gPlayers) {
            // 1.00 es a SkyWarsReloaded.getCfg().preGameTimer()
            // lo que X es a lvl
            // X = lvl * 1.00 / SkyWarsReloaded.getCfg().preGameTimer()
            gpl.getP().setExp(lvl / SkyWarsReloaded.getCfg().preGameTimer());
            gpl.getP().setLevel(lvl);
        }
    }
