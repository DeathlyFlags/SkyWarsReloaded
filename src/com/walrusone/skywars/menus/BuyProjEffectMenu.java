...
public class BuyProjEffectMenu {
            ...
            public void onOptionClick(IconMenu.OptionClickEvent event) {
                ...
                if (canPurchase(gamePlayer, effect)) {
                    ...
                    if (hasEffectPermission(gamePlayer, effect)) {
                        ...
                        if (!gamePlayer.inGame()) {
                            SkyWarsReloaded.get().getServer().getScheduler().scheduleSyncDelayedTask(SkyWarsReloaded.get(), () -> updateBuyEffectMenu(gamePlayer), 2);
                        }
                    }
                }
            }
    ...
    private void updateBuyEffectMenu(final GamePlayer gamePlayer) {
    ...
    private boolean hasEffectPermission(GamePlayer player, ParticleItem effect) {
    ...
    private boolean canPurchase(GamePlayer gamePlayer, ParticleItem effect) {
        if (SkyWarsReloaded.getCfg().usingExternalEcomony()) {
