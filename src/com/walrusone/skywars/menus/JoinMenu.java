...
public class JoinMenu {
    ...
    private static final String joinName = new Messaging.MessageFormatter()
            .format("menu.joingame-menu-title");
    ...
    public JoinMenu(final GamePlayer gamePlayer) {
        ...
        SkyWarsReloaded.getIC().create(gamePlayer.getP(), joinName, rowCount,
                event -> {
                    if (gamePlayer.inGame()) {
                        return;
                    }

                    Game game = SkyWarsReloaded.getGC().getGame(
                            Integer.parseInt(ChatColor.stripColor(event
                                    .getName())));
                    if (game == null) {
                        return;
                    }
        ...
        for (int iii = 0; iii < games.size(); iii++) {
            ...
            if (game.getState() != GameState.PREGAME) {
                loreList.add((new Messaging.MessageFormatter()
                        .format("signJoinSigns.inprogress").toUpperCase()));
            }
            loreList.add(ChatColor.RED + "" + ChatColor.BOLD
                    + game.getMapName().toUpperCase());
            loreList.add((new Messaging.MessageFormatter()
                    .format("menu.spectate-game-header")
                    + "  "
                    + game.getPlayers().size() + "/" + game.getNumberOfSpawns()));
            ...
            double xy = ((double) (game.getPlayers().size() / game
                    .getNumberOfSpawns()));
            ...
            if (gamePlayer.getP() != null) {
                SkyWarsReloaded.getIC().setOption(gamePlayer.getP(), iii,
                        new ItemStack(gameIcon, 1), gameNumber,
        ...
        SkyWarsReloaded.get().getServer().getScheduler()
                .scheduleSyncDelayedTask(SkyWarsReloaded.get(), () -> updateJoinMenu(gamePlayer), 40);
    }

    private void updateJoinMenu(final GamePlayer gamePlayer) {
        if (SkyWarsReloaded.getIC().has(gamePlayer.getP())
                || gamePlayer.inGame()
                || !SkyWarsReloaded.getIC().getMenu(gamePlayer.getP())
                .getName().equalsIgnoreCase(joinName)) {
        } else {
            List<Game> games = SkyWarsReloaded.getGC().getGames();
